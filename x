...
local function GetProximityLimit()
    local proximityLimit = 100  -- Default value

    if getgenv().forbidden.PointLock.ProximityLimit == "powerful" then
        proximityLimit = 200
    elseif getgenv().forbidden.PointLock.ProximityLimit == "basic" then
        proximityLimit = 100
    end
...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            -- Removed Circle.Visible line to ensure it is never visible
            Circle.Radius = GetProximityLimit()  -- Use the function to get ProximityLimit
...
local function GetClosestBodyPart(character)
    ...
    local proximityLimit = GetProximityLimit()  
    ...
    if character and character:IsDescendantOf(game.Workspace) then
        for _, part in ipairs(character:GetChildren()) do
            if part:IsA("BasePart") then
                ...
                if OnScreen then
                    local Distance = (Vector2.new(Position.X, Position.Y) - Vector2.new(Mouse.X, Mouse.Y)).Magnitude
                    if proximityLimit > Distance and Distance < ClosestDistance then
